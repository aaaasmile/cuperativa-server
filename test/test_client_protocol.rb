...
class Test_Client_Protocoll < Test::Unit::TestCase
  ...
  def test_updateresp2_platform
    ...
    msg_details = JSON.generate({ :type => typesym, :link_platf => linkplat, :server => serverstr, :file => file_pack_str, :size => sizestr })
  ...
  def test_pgadd2
    ...
    msg_details = JSON.generate({ :index => ix, :user => user, :game => gamesym,
