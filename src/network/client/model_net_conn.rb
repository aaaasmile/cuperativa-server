...
class ModelNetData
  ...
  def state_exit
    @log.debug("Net_state: change to state state_exit")
    @network_state = :state_exit
    make_state_change_ntfy(:ntfy_state_exit)
  end
  ...
  #           slice => 0...N
  #           detail => [{....pg_add2 hash ...}] same of the pg_add2},..., ]
  #           }
  def parse_list2(msg_details)
    ...
    type_list = msg_details["type"].to_sym
    slice_nr = msg_details["slice"]
    slice_state = msg_details["slice_state"].to_sym
