...
class ControlNetConnection
  ...
  def send_data_to_server(data)
    if @socket_srv != nil
      ...
      @log.debug "Ignore send message on disconnected server"
    end
  rescue Timeout::Error
  ...
  def cmdh_update_resp2(msg_details)
    ...
    case info[:type] 
      ...
      when :appli_update
        ...
        url_port = 80
        port_splitted = server_name.split(':')
        if port_splitted.size > 1
          url_port =  port_splitted[1]
          server_name = port_splitted[0]
        end
        @log.debug "requesting url #{server_name}, #{url_port}, #{package_name}"
        updater = ClientUpdaterCtrl.new
        updater.set_package_src(server_name, package_name, url_port)
