...
class ClientUpdaterCtrl
  ...
  def initialize
    ...
    @update_prg_ind = 0
    @updater_yaml_interface = "updater_process"
  end
  
  #url: example ->  'http://127.0.0.1:4561/update_package_onlycode_10_9_0.tar.gz'
  def set_package_src_withfullurl(url)
    server_name = '', package_name = '', http_port = '80'
    p uri = URI::parse(url)
    server_name =  uri.host
    http_port =  uri.port
    package_name =  uri.path
    set_package_src(server_name, package_name, http_port)
  end
  
  ##
  # Set info about server and package
  def set_package_src(server_name, package_name, http_port)
    @url_server_name_pkg = server_name # NOTE: don't use http://
    @url_file_name_pkg = package_name
    @url_port = http_port
    @log.debug "url on package is http://#{server_name}:#{http_port}#{package_name}"
  ...
  def download_update_package
    @log.debug "Downloading from server #{@url_server_name_pkg} on port #{@url_port}"
    http=Net::HTTP.start(@url_server_name_pkg, @url_port)
    
    @log.debug "Request package #{@url_file_name_pkg}"
    ...
    when Net::HTTPMovedPermanently, # 301
         ...
         # follow redirect
         loc = resp['location']
         @log.debug "Moved to another url: #{loc}"
         set_package_src_withfullurl(loc)
         download_update_package
    else
      raise "Package to download #{@url_file_name_pkg} not found on server #{@url_server_name_pkg}, http error: #{resp.message}"
    end
  end
  ...
  def execute_manifest
    ...
    # open updater_process
    updater_dir = File.join(File.dirname(__FILE__), "../../../../updater")
    updater_yamlname = File.expand_path(File.join(updater_dir, @updater_yaml_interface))
    updt = {}
    if File.exist?(updater_yamlname)
      updt = YAML::load_file( updater_yamlname )
    end
    
    updt[:process_manifest] = true
    updt[:manifest_dir] = root_new_files
    # write back, updater process will use it
    ...
    @log.debug "Version in manifest #{@new_version_str}"
  rescue
...
if $0 == __FILE__
  ...
  updater.set_package_src('127.0.0.1', '/cuperativa/update_packages/update_package_onlycode_10_9_0.tar.gz', 4560)
  ...
  #updater.set_package_src_withfullurl('http://127.0.0.1:4561/update_package_onlycode_10_9_0.tar.gz')
