...
require 'base/gfx_general/table_seat_gfx'
...
class NetworkCockpitView
  ...
  def initialize(comment,  split_horiz_netw, gui_owner, controller, data_model)
    ...
    @splitter_tavolo = FXSplitter.new(main_vertical, (LAYOUT_SIDE_TOP|LAYOUT_FILL_X|
                       LAYOUT_FILL_Y|SPLITTER_VERTICAL|SPLITTER_TRACKING))
    
    # Table
    tb_vertical = FXVerticalFrame.new(@splitter_tavolo, 
                           FRAME_THICK|LAYOUT_SIDE_TOP|LAYOUT_FILL_X|LAYOUT_FILL_Y)
    @table_games = FXTable.new(tb_vertical, nil, 0,
    ...
    # canvas tavolo
    @canvas_disp = FXCanvas.new(@splitter_tavolo, nil, 0, LAYOUT_FILL_X|LAYOUT_FILL_Y|LAYOUT_BOTTOM|LAYOUT_LEFT )
    ...
    @canvas_disp.backColor = Fox.FXRGB(0x22, 0x8a, 0x4c)
    # double buffer image for canvas
    @imgDbuffHeight = 0
    @imgDbuffWidth = 0
    @image_double_buff = nil
    @seat_gfx = TableSeatGfx.new(self, @cup_gui, @canvas_disp.height, @canvas_disp.width )
    
    # pulsanti 
    btframe = FXHorizontalFrame.new(tb_vertical, 
    ...
    ## logger for debug
    @log = Log4r::Logger.new("coregame_log::NetworkCockpitView") 
    #
  end
  ...
  def onLMouseUp(sender, sel, event)
    @seat_gfx.onLMouseUp(event)
  end
  
  def onLMouseDown(sender, sel, event)
    @seat_gfx.onLMouseDown(event)
  end
  ...
  def onCanvasPaint(sender, sel, event)
    ...
    @seat_gfx.draw_static_scene(dc,@image_double_buff.width, @image_double_buff.height)
    dc.end
  ...
  def OnCanvasSizeChange(sender, sel, event)
    #@log.debug("OnSizeChange w:#{@canvas_disp.width}, h:#{@canvas_disp.height}\n")
    ...
    if adapt_to_canvas
      ...
      @image_double_buff = FXImage.new(@cup_gui.MainApp, nil, 
             IMAGE_SHMI|IMAGE_SHMP, @imgDbuffWidth, @imgDbuffHeight)
      @image_double_buff.create
      #notify change
      begin
        @seat_gfx.onSizeChange(@imgDbuffWidth, @imgDbuffHeight ) if @seat_gfx
      rescue => detail
  ...
  def table_games_sel_change(sender, sel, ptr)
    ...
    if ix_pg
      ...
      @seat_gfx.set_game_detail(@game_info_det[ix_pg])
      # this 2 lines below are used to select the full row
  ...
  def table_remove_pg_game(ix_to_remove)
    (0..@table_games.numRows-1).each do |r_ix|
      ...
      if ixpg_on_row.to_i == ix_to_remove
        ...
        @game_info_det.delete(ix_to_remove)
        @seat_gfx.set_game_detail(nil)
