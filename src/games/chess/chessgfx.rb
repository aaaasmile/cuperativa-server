...
class ChessGfx 
  ...
  def start_new_game(players, options)
    ...
    @board_gfx = BoardChessGfx.new(@image_gfx_resource, 20, 20)
    
    players.each do |pl|
      if pl.type == :cpu_alg
        pl.algorithm = AlgCpuChessDummy.new(pl, @chess_core, nil)
      elsif pl.type == :human_local
        @gui_player = pl
        pl.algorithm = self
      end
    end
  ...
  def onalg_new_match(players_info)
    @log.debug "onalg_new_match"
    @board_gfx.onalg_new_match(players_info)
  ...
  def onalg_have_to_move(player)
    @log.debug "onalg_have_to_move"
    @board_gfx.onalg_have_to_move(player)
  ...
  def onalg_player_has_moved(player, move )
    ...
    @board_gfx.onalg_player_has_moved(player, move)
    
    @cupera_gui.update_dsp 
  end
  
  #reason:  :not_in_turn
  def on_alg_hasmoved_invalid(move, reason)
    @log.debug "Played an invalid move #{move.move_to_str}, reason#{reason}"
  end
  ...
  def do_move_to_core
    ...
    move = @board_gfx.dragging_piece[:move]
    # TODO
    #@chess_core.alg_make_move(col_start, row_start, col_end, row_end)
    @chess_core.alg_player_move(@gui_player, move)
  end
  
  def do_core_process
    @chess_core.process_only_one_gevent if @chess_core
  end
...
if $0 == __FILE__
  ...
  player2 = PlayerOnGame.new('Dummy', nil, :cpu_alg, 1)
  players << player2
