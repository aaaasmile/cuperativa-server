...
class AlgCpuChess 
  ...
  def strip_pos_argument(argument)
    return argument[:color], argument[:start_x],argument[:start_y],argument[:end_x],argument[:end_y], argument[:move_type]
  end
  ...
  def alg_make_move
    ...
    if move_info_arr.size == 4
      ...
      info = {:color => @alg_color, :start_x => start_x, :start_y => start_y, :end_x => end_x, :end_y => end_y, :move_type => :move}
      @core_game.alg_player_move( info )
    else
