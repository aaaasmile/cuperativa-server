# file: nal_srv_tombolon.rb.rb
# Network abstraction layer on server for game tombolon
...
require File.dirname(__FILE__)  + '/../../src/games/tombolon/core_game_tombolon'


module MyGameServer
  
##
# Instance used by server to manage the tombolon core game  
class NALServerCoreGameTombolon < NalServerCoreBase
   
  def initialize(ix, dir_log)
    super("tombolon2p", ix, dir_log)
    @log = Log4r::Logger.new("coregame_log::NALServerCoreGameTombolon") 
    @name_core_game = :tombolon_game
    # NAL_Srv_SpazzAlgo is the nal for all scopa, tombolon and  similar games
    @nal_algorithm_name = 'NAL_Srv_SpazzAlgo' 
    @gamename_indb = 'Tombolon'
    @core_game_name = 'CoreGameTombolon'
  end
...
end #end class NALServerCoreGameTombolon
